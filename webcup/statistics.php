...
<div class="container">
  <header>
   ...
   <h1 style="background-color: orange" align="center">Statistics Based on Departments</h1>
  ...
  $connect = mysqli_connect($dbhost, $dbuser, $dbpass, $dbname) or die(mysqli_error($connect));
      ...
      $query1= mysqli_query($connect,"SELECT COUNT(*) FROM `users` WHERE fieldOfStudy ='Engineering'");
      while($row1 = mysqli_fetch_assoc($query1)){
      foreach($row1 as $cname1 => $cvalue1){
      }}

      $query2= mysqli_query($connect,"SELECT COUNT(*) FROM `users` WHERE fieldOfStudy ='Law'");
      while($row2 = mysqli_fetch_assoc($query2)){
      foreach($row2 as $cname2 => $cvalue2){
      }}

      $query3= mysqli_query($connect,"SELECT COUNT(*) FROM `users` WHERE fieldOfStudy ='Medicine'");
      while($row3 = mysqli_fetch_assoc($query3)){
      foreach($row3 as $cname3 => $cvalue3){
      }}

      $query4= mysqli_query($connect,"SELECT COUNT(*) FROM `users` WHERE fieldOfStudy ='Hospitality'");
      while($row4 = mysqli_fetch_assoc($query4)){
      foreach($row4 as $cname4 => $cvalue4){
      }}

      $query5= mysqli_query($connect,"SELECT COUNT(*) FROM `users` WHERE fieldOfStudy ='Finance'");
      while($row5 = mysqli_fetch_assoc($query5)){
      foreach($row5 as $cname5 => $cvalue5){
      }}
      $query6= mysqli_query($connect,"SELECT COUNT(*) FROM `users` WHERE fieldOfStudy ='Accounting'");

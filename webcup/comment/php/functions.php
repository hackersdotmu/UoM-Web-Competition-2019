<?php
	...
	function get_total() {
		...
		echo '<h1>Project ('.$row_cnt.')</h1>';
	}
	//function deletecomms() {
	//	require 'connect.php';
	//	$result = mysqli_query($connect, "DELETE FROM `parents` WHERE COMMENT ="'.$comment.'" AND id="'.$par_code.'"");
	//	$row_cnt = mysqli_num_rows($result);
	...
	function get_comments() {
		...
		foreach($result as $item) {
			...
			echo '<div class="comment" id="'.$par_code.'">'
					...
					.'<a class="link-reply" id="reply" name="'.$par_code.'">Reply</a>&nbsp'
					.'<a class="link-reply" id="reply" name="'.$par_code.'">Delete</a>'; 
					require 'likebut/phplikes/index.php';
